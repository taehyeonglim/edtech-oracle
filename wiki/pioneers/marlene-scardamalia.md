...
> 학습자는 이미 만들어진 지식을 이해하는 데서 더 나아가 공동체가 사용할 새 지식을 개선할 수 있다.[^scardamalia-1994]

## 주요 저작

- **Computer-Supported Intentional Learning Environments** (1989) — 네트워크 데이터베이스로 의도적 학습을 지원한 CSILE의 설계 원리.[^scardamalia-1989]
- **Computer Support for Knowledge-Building Communities** (1994) — 학교 담화를 공동 지식의 진보로 재구성한 핵심 논문.[^scardamalia-1994]
...
스카다말리아와 베라이터는 지식구축을 공동체가 공적 아이디어를 지속적으로 개선하는 활동으로 설명했다.[^scardamalia-1994] 에티엔 웽거는 학습을 공동 실천에 참여하며 의미·소속·정체성을 협상하는 과정으로 분석했다. 이 반론은 아이디어의 개선만을 분석 단위로 삼을 때 참여의 방식과 학습자의 정체성 형성이 충분히 포착되지 않을 수 있음을 보여준다.[^wenger-1998]

시모어 파퍼트는 컴퓨터로 공유 가능한 산출물을 만들고 그 과정에서 자신의 사고를 외부화·재구성하는 학습자를 강조했다.[^papert-1980] 이는 공동 담화 속 아이디어의 진보를 중심에 두는 지식구축과 달리, 개별 학습자의 만들기와 구성물에 대한 관계를 학습의 중요한 출발점으로 본다.[^papert-1980][^scardamalia-1994]

## 한계

스카다말리아의 지식구축과 CSILE은 학습자가 글·그림·그래프를 공용 데이터베이스에 남기고, 공동체가 그 아이디어를 연결·논평·수정하는 상황을 중심으로 설계되었다. 따라서 공유할 문제와 공적 표상, 지속적인 수정 담화가 마련되지 않는 학습 상황까지 이 모형 하나로 설명하거나 처방하기는 어렵다.[^scardamalia-1989][^scardamalia-1994]

이론의 성공을 게시물 수나 연결 수로 대체하는 것도 오용이다. 지식구축의 기준은 기록이 쌓였다는 사실이 아니라 공동체의 아이디어가 더 나은 설명으로 개선되었는지에 있으며, 참여·소속·정체성의 변화는 웽거가 제시한 별도의 분석 차원으로 남는다.[^scardamalia-1994][^wenger-1998]

## 연표

- **1989** CSILE — 의도적 학습을 지원하는 초기 네트워크형 지식 매체의 원리를 공개했다.[^scardamalia-1989]
- **1994** Knowledge-Building Communities — 학교를 공동 지식 창출 공동체로 재구성하는 이론을 발표했다.[^scardamalia-1994]
- **1995** National Academy of Education — 지식구축과 학습기술 연구의 공헌으로 학술원 회원에 선출됐다.[^bio-scardamalia-nae]
