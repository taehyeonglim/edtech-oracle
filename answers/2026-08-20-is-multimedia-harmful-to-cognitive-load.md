...
rounds_run: 3
note: >-
  라운드 1(입장 표명)·2(반박)·3(쟁점 정리)을 모두 진행했다. 문서 순서가 라운드 순서다 — 각 slug의 첫 섹션이 라운드 1, 두
  번째 섹션이 라운드 2이며, 마지막 `_orchestrator` 섹션이 라운드 3이다. 세 라운드는 각각 별도 세션 턴에서 실행됐다.
check:
  status: violations
  forge: 0
  form: 70
...
**"멀티미디어 자체가 인지부하에 해로운가"라는 명제형 질문은 네 명 모두 기각했다.** 라운드 1에서 넷은 서로의 발언을 보지 못한 채 독립적으로 이 질문을 거부했다. 스웰러는 "해롭다/해롭지 않다의 이분법을 거부한다"고 명시했고, 메이어는 자기 입장이 "잘못 설계된 멀티미디어가 외재적 인지부하를 늘려 학습을 방해할 수 있다"는 조건부 명제라고 못박았으며, 판 메리엔보어는 질문을 "어느 구성요소에서 어떤 기능을 수행하도록 설계되었는가"로 바꿔 물어야 한다고 했고, 코즈마는 매체를 독립변수로 놓는 질문 형식 자체가 오류라고 했다. 라운드 2에서 코즈마는 나머지 셋이 이 결론으로 수렴했다고 직접 확인했다.

**주의분할 현상 자체는 스웰러와 메이어가 서로의 실험을 같은 현상으로 인정했다.** 메이어는 라운드 2에서 스웰러와 챈들러의 결과가 자신이 모레노와 검증한 것과 "같은 현상을 가리킨다"고 했고, "형식이 부하를 결정한다"는 스웰러의 결론에 동의한다고 명시했다. 두 사람이 이 현상의 존재를 놓고 다투는 지점은 이 토론에 없다.

**부하를 줄이는 것 자체가 목표가 아니라는 데 스웰러·메이어·판 메리엔보어가 같은 말을 했다.** 스웰러는 목표가 "최소화가 아니라 학습자의 사전지식에 맞게 재배분하는 것"이라 했고, 판 메리엔보어는 외생부하는 줄이고 학습 관련 부하는 "필요할 때 늘려야 한다"고 했으며, 메이어는 자기 원리들이 "과제 자체의 난이도를 없애는 것을 목표로 삼지 않았다"고 했다. 코즈마는 이 논점에 참여하지 않았다 — 부하의 삼분법 자체를 자기 문헌에 없다고 선언했기 때문이다.

**인지부하 이론이 표상 설계 이론은 아니라는 점을 스웰러 본인이 인정했다.** 스웰러는 조나센의 비판을 인용해 "내 이론은 부하를 관리하는 조건을 특정하지만, 표상을 어떻게 구성하고 연결할지에 대한 설계 이론은 아니다"라고 했다. 이는 코즈마가 요구한 방향과 겹치지만, 스웰러는 이를 코즈마 편에 서는 진술이 아니라 자기 이론의 범위 한정으로 말했다.

### 미해결 대립축

**1. 이중 채널의 축이 감각 양식인가 처리 양상인가 — 스웰러 대 메이어.** 스웰러는 반 메리엔보어·파스와 함께 작업기억을 청각·언어와 시각·공간이라는 감각 양식으로 나눴다고 했다. 메이어는 라운드 2에서 "내가 다루는 이중 채널은 청각/시각이 아니라 언어/시각"이라며, 텍스트든 내레이션이든 언어적으로 처리되면 언어 채널을 거친다고 선을 그었다. 그는 두 정식화가 "같은 방향을 가리킨다"고만 했지, 어느 쪽이 옳은지는 다루지 않았다. 이 축이 화해되지 않은 이유는 논쟁이 붙었다가 결렬됐기 때문이 아니라 **대면되지 않았기 때문이다** — 라운드 2는 상대의 라운드 1 발언만 프롬프트로 받으므로, 먼저 발언한 스웰러는 메이어의 이 구분에 답할 기회가 없었다. 토론 설계가 만든 공백이며, 사회자가 메울 자리가 아니다.

**2. 부하 프레임이 이 질문에 충분한가 — 코즈마 대 나머지 셋.** 코즈마는 셋 모두 "매체를 작업기억이 처리해야 할 부담의 근원으로만 다룬다"고 지적하고, 매체의 표상·연산이 학습자의 인지·사회적 과정과 어떻게 결합하는가라는 질문은 "부하량 계산으로 환원되지 않는다"고 했다. 세 사람의 응답은 각기 다르다. 메이어는 층위가 다를 뿐 "상충하기보다 서로 다른 층위에서 같은 현상을 가리킨다"며 양립을 주장했다. 스웰러는 자기 이론이 표상 설계 이론이 아님을 인정하되 그 공백을 조나센 쪽으로 넘겼다. 판 메리엔보어는 정면으로 반대 설명을 냈다 — 학생이 표상을 연결하지 못하는 것은 "표상이 방치된 설계 결함이 아니라, 필요한 정교화가 아직 자동화되지 않은 초보자 상태 자체의 특징일 수 있다"는 것이다. 같은 관찰(학생의 표상 간 연결 실패)에 네 개의 서로 다른 소속 판정이 붙었고, 어느 판정도 상대의 문헌으로 반증되지 않았다.

**3. 어떤 종류의 증거가 이 질문에 답하는가 — 실험실 인과 대 현장 연관.** 메이어는 자기 결론의 근거 대부분이 짧고 통제된 실험실 과제에서 나왔고 생태 타당도가 "이 연구 프로그램이 구조적으로 지불하는 비용"이라고 스스로 인정했다. 판 메리엔보어는 10차시 고등학교 프로그래밍 연구를 실험실 밖 증거로 보탰지만, 협력 과제나 몇 주 단위 프로젝트형 학습까지의 일반화는 "내 문헌에도 없다"고 못박았다. 코즈마는 28개국 174건 사례 분석을 내놓으면서 "인과를 증명하는 자료가 아니라 연관을 보고하는 자료"이며 이를 실험실 원리를 반박하는 근거로 쓰지 않는다고 스스로 제한했다. 셋 다 자기 증거의 한계를 먼저 밝혔고, 서로 다른 층위의 증거를 같은 저울에 올릴 방법은 아무도 제시하지 않았다. 이 축이 미해결인 것은 증거가 부족해서가 아니라 비교 기준이 없어서다.

**4. 인지부하 개념의 귀속 — 코즈마의 라운드 1 진술이 정정된 채 남았다.** 코즈마는 라운드 1에서 인지부하라는 용어와 내재적/외재적 구분이 "메이어 계열의 프레임워크에서 온 것"이라고 했다. 스웰러는 라운드 2에서 이를 정면으로 정정하며 1988년 자기 논문이 먼저라고 했고, 메이어 자신도 라운드 1에서 자기 설계 원리가 "스웰러의 인지부하 이론에서 왔다"고 밝혔다. 순차 구조상 코즈마는 스웰러의 정정을 받지 못했으므로 이 지점에 대한 그의 답변은 이 토론에 없다. 사회자는 누가 옳은지 판정하지 않되, 세 발언이 이 관계에 놓여 있다는 사실은 기록해 둔다.

**5. 컴퓨터 기반 학습이라는 질문 조건에 대해 네 명 모두 원리 수준으로만 답했다.** 스웰러는 자기 원 실험 다수가 "반드시 컴퓨터 화면은 아니었다"고 밝혔고, 판 메리엔보어는 자기 문헌이 다룬 범위가 1990~2010년대 컴퓨터 기반 훈련과 의료교육까지이며 그 이후 적응형 플랫폼이나 몰입형 시뮬레이션은 아니라고 했으며, 메이어와 코즈마는 오늘날의 도구에 대한 발언을 각각 `[적용]`으로 표시했다. 이 질문의 현대적 부분에 대해 네 사람이 가진 것은 원리로부터의 추론이지 직접 검증이 아니다.

### 메우지 않은 빈칸

이 토론에는 `[근거없음]`이 일곱 번 나왔다. 라운드 1에서 메이어는 생성형 AI가 만든 자료에 대해, 판 메리엔보어는 메이어 계열의 개별 원리 실험에 대해, 코즈마는 인지부하 이론 내부 기제 전반에 대해 근거 없음을 선언했다. 라운드 2에서 스웰러는 자신이 수행하지 않은 상대들의 실험에 대해, 메이어는 전문성 역전 효과와 4C/ID 모형에 대해, 코즈마는 작업기억 이중 채널 구조에 대해 같은 선언을 했다.

특히 주의할 자리가 하나 있다. 메이어는 전문성 역전 효과를 검증할 근거가 자기 문헌에 없다고 비웠는데, 같은 효과를 스웰러는 자기 이론의 핵심 결과로, 판 메리엔보어는 "핵심 설계 제약"으로 확언했다. 이 빈칸을 두 사람의 근거로 메우면 세 사람이 합의한 것처럼 보이지만, 메이어의 원리 체계 안에서 그 효과는 여전히 검증되지 않은 채다. 같은 이유로 코즈마가 비운 이중 채널 구조의 자리도 스웰러의 문헌으로 채우지 않았다.
