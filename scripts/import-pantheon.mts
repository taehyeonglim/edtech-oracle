...
/**
 * 출처 페이지의 짧은 티어 설명. `CLAUDE.md`의 티어 표와 같은 경계를 말해야 한다.
 *
 * **이 임포터는 실행하지 않는다.** 위인 페이지 36개가 있으면 스스로 멈추는 일회성
 * 부트스트랩이고 `--force`는 현재 위키를 다시 만든다. 그래도 문구를 고쳐 두는 것은,
 * 옛 문구를 남겨 두면 훗날 강제 재생성 때 폐기된 A/B 경계가 되살아나기 때문이다.
 */
const TIER_NOTE: Record<string, string> = {
  A: "위인 본인의 원저작 — 형식 불문 · 당사자 기록 · 원문 아카이브",
  B: "후대의 종합·편집·교재·번역 · 타인의 학술 문헌 · 기관 공식 기록",
