...
합계 5명 · 6건. 시작 전
`npm run verify:source-expansion`이 "새 id 110건, 완료 감사 26건"으로
통과하는지 확인하라.

**요청 후보 수는 `max(2n, n+3)`이다.** 부족분 1이면 4건, 2면 5건.

**이 다섯이 가장 어렵다.** 스키너·브루너·비고츠키는 교육공학사에서 문헌이 가장
많은 인물인데 바로 그래서 대표 저작이 이미 9건 등재돼 있다. 남은 1건은 유명한
것이 아니라 **아직 안 들어간 것**이어야 한다. 중복 색인(252건)을 반드시 확인하라.
중복이면 `rejected`다 — "확인 못 함"이 아니라 "이미 있음"이다.

**못 채우면 보류하고 넘어가라.** 26명이 보류 없이 채워졌지만 그것이 목표가
아니다. 비고츠키에게 10번째 출처를 붙일 수 없다면 그것이 사실이고, 억지로
채우면 근거는 늘지 않은 채 숫자만 맞는다.

**`john-keller`(+1)는 고위험 동명이인이다.** 독립 식별 신호 2개와 강한 신호
1개를 요구한다. `allan-collins`는 BBN 소속·공저자망으로, `richard-clark`은 USC
소속과 Salomon 공저 관계로 통과했다. 같은 수준이다.
