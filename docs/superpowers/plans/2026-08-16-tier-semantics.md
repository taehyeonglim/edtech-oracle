...
- **규칙 11은 `scripts/site/collect.mjs`의 `assertSiteInvariants`가 이미 하던 각주 검사를 흡수한다.** 명세는 "`lint-wiki.mjs`에 tier 표기 규칙이 없다"를 근거로 규칙 11을 세웠는데, 그 능력은 사이트 어서션 계층(`collect.mjs:255-259`)에 이미 있었다. 실측하면 규칙 11은 티어 불일치뿐 아니라 각주 라벨과 꼬리 링크 불일치(`[^a]: … [[sources/b]]`)까지 잡아 어서션의 각주 검사를 완전히 덮는다. **규칙 11은 유지한다** — 커밋 게이트인 `lint:strict`에서 잡히는 편이 빌드 때만 도는 어서션보다 이르고, 과제 4의 전파 누락을 드러내려면 커밋 게이트에 있어야 한다. 대신 어서션 계층에 남은 고유 불변식은 레이아웃 오류와 **debate `related` 개수**뿐이므로(`lint-wiki.mjs`에는 `related`를 보는 규칙이 없다), `사이트 어서션 위반도 빌드를 막는다`는 과제 2에서 `related`가 1개인 debate 픽스처로 옮긴다. 옮기지 않으면 lint가 먼저 막아 어서션 계층이 **무검사 상태**가 된다.
